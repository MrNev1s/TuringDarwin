...

{
  ...
  echo "turingprobe_version=0.7.0"
  echo "mmio_compile_gate=TURINGPROBE_ENABLE_MMIO_READ=1"
  echo "fb_compile_gate=TURINGPROBE_ENABLE_FB_READ=1"
  echo "host_memory_compile_gate=TURINGPROBE_ENABLE_HOST_MEMORY_TEST=1"
  echo "host_physical_compile_gate=TURINGPROBE_ENABLE_HOST_PHYSICAL_TEST=1"
  ...
  echo "host_memory_mode=-tdprobe+-tdhostmem-test"
  echo "host_physical_mode=-tdprobe+-tdhostphys-test"
  echo "host_physical_descriptor=IOBufferMemoryDescriptor;capacity=4096;alignment=4096;mapper=none;direction=none"
  echo "host_physical_raw_segment_queries=1"
  echo "host_physical_validation=nonzero;page-aligned;length=4096;within-47-bits;zero-before-release"
  echo "host_memory_test=3x4096:prefix-canary+payload+suffix-canary;alignment=4096;cpu-write-readback;zero-before-free"
  echo "device_memory_write_whitelist=EMPTY"
  echo "mmu_hardware_whitelist=EMPTY"
  echo "offline_validation=golden-vectors;mmu-model;page-table-images;address-space;transaction-rollback;host-memory-model;host-memory-kext-contract;host-physical-model;host-physical-kext-contract"
